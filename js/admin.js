...
const Admin = (() => {
  ...
  async function saveMission() {
    ...
    const body = {
      ...
      type: document.getElementById("mis-type")?.value || document.getElementById("dev-mission-type")?.value || "daily",
      condition: document.getElementById("mis-condition")?.value,
      conditionValue: parseInt(document.getElementById("mis-condition-value")?.value) || 1,
    ...
    if (!body.condition) { UI.toast("أدخل شرط المهمة", "error"); return; }
    try {
      ...
      if (res.ok && (d.success !== false)) {
        ...
        UI.toast(d.error || d.message || `خطأ ${res.status}: ${res.statusText}`, "error");
      }
    } catch (e) { UI.toast("فشل حفظ المهمة: " + e.message, "error"); }
  }

  async function sendMessageToPlayer() {
    const playerId = document.getElementById("dev-msg-player-id")?.value.trim();
    const msg = document.getElementById("dev-msg-content")?.value.trim();
    if (!playerId) { UI.toast("أدخل معرف اللاعب", "error"); return; }
    if (!msg) { UI.toast("أدخل نص الرسالة", "error"); return; }
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`${API}/admin/message/${playerId}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ message: msg }),
      });
      ...
      if (res.ok) {
        ...
        if (document.getElementById("dev-msg-player-id")) document.getElementById("dev-msg-player-id").value = "";
        if (document.getElementById("dev-msg-content")) document.getElementById("dev-msg-content").value = "";
      } else {
        UI.toast(d.error || "فشل الإرسال", "error");
      }
    } catch { UI.toast("فشل الإرسال", "error"); }
  }
  ...
  return {
    ...
    broadcast, sendNotification, sendMessageToPlayer,
