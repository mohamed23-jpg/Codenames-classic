...
const Game = (() => {
  ...
  // تحديث بطاقات الوضع عند تغيير نوع اللعبة
  function onGameModeChange(mode) {
    const cardGroup = document.getElementById("card-count-options");
    if (!cardGroup) return;
    if (mode === "solo") {
      cardGroup.innerHTML = `
        <label class="radio-label"><input type="radio" name="card-count" value="15" checked /> 15 بطاقة</label>
        <label class="radio-label"><input type="radio" name="card-count" value="20" /> 20 بطاقة</label>
        <label class="radio-label"><input type="radio" name="card-count" value="25" /> 25 بطاقة</label>
      `;
    } else {
      cardGroup.innerHTML = `
        <label class="radio-label"><input type="radio" name="card-count" value="25" checked /> 25 (5×5)</label>
        <label class="radio-label"><input type="radio" name="card-count" value="16" /> 16 (4×4)</label>
        <label class="radio-label"><input type="radio" name="card-count" value="12" /> 12 (3×4)</label>
      `;
    }
  }

  // تحديث بطاقات الوضع في اللوبي
  function onLobbyModeChange() {
    const mode = document.getElementById("lobby-mode")?.value;
    const sel = document.getElementById("lobby-card-count");
    if (!sel || !mode) return;
    sel.innerHTML = mode === "solo"
      ? `<option value="15">15 بطاقة</option><option value="20">20 بطاقة</option><option value="25">25 بطاقة</option>`
      : `<option value="25">25 (5×5)</option><option value="16">16 (4×4)</option><option value="12">12 (3×4)</option>`;
    const socket = SocketClient.getSocket();
    socket?.emit("update_room_mode", { code: currentRoom?.code, mode, cardCount: parseInt(sel.value) });
  }
  ...
  function createRoom() {
    ...
    const visibility = document.querySelector('.visibility-option.selected')?.dataset.value || "public";
    ...
    socket.emit("create_room", {
      name, password, cardCount, mode, isPrivate,
  ...
  function renderTeamSlot(elId, player, role) {
    ...
    if (!el) return;
    const me = App.getPlayer();
    const isHost = currentRoom && (currentRoom.createdBy === me?.playerId || currentRoom.host === me?.playerId);
    if (!player) {
      el.innerHTML = `<span class="slot-empty">فارغ</span>`;
    } else {
      const isMe = player.playerId === me?.playerId;
      const canKick = (isHost || me?.isDev) && !isMe;
      el.innerHTML = `
        <div class="player-in-slot">
          <div class="slot-avatar">${Effects.buildAvatarImg(player.avatar, 30)}</div>
          <div class="slot-info">
            <div class="slot-name">${player.nickname}${isMe ? ' <span class="member-is-you">(أنت)</span>' : ''}</div>
            <div class="slot-level">LV.${player.level || 1}${player.activeTitle ? ` · ${player.activeTitle}` : ''}</div>
          </div>
          ${canKick ? `<button class="btn btn-danger btn-kick-slot" onclick="Game.kickFromSlot('${player.playerId}')">طرد</button>` : ''}
        </div>
      `;
    }
  }
  ...
  function renderTeamSlots(elId, players) {
    ...
    const isHost = currentRoom && (currentRoom.createdBy === myId || currentRoom.host === myId);
    el.innerHTML = players.map((p) => {
      const isMe = p.playerId === myId;
      const canKick = (isHost || me?.isDev) && !isMe;
      return `
        <div class="player-in-slot">
          <div class="slot-avatar">${Effects.buildAvatarImg(p.avatar, 30)}</div>
          <div class="slot-info">
            <div class="slot-name">${p.nickname}${isMe ? ' <span class="member-is-you">(أنت)</span>' : ''}</div>
            <div class="slot-level">LV.${p.level || 1}${p.activeTitle ? ` · ${p.activeTitle}` : ''}</div>
          </div>
          ${canKick ? `<button class="btn btn-danger btn-kick-slot" onclick="Game.kickFromSlot('${p.playerId}')">طرد</button>` : ''}
        </div>
      `;
    }).join("") || `<span class="slot-empty">فارغ</span>`;
  }

  function kickFromSlot(targetPlayerId) {
    const socket = SocketClient.getSocket();
    if (!currentRoom) return;
    socket?.emit("kick_from_slot", {
      code: currentRoom.code,
      targetPlayerId,
    }, (result) => {
      if (result?.success) UI.toast("تم طرد اللاعب من المنصب", "success");
      else UI.toast(result?.error || "تعذّر الطرد", "error");
    });
  }
  ...
  return {
    ...
    kickFromSlot, renderTeamSlot, renderTeamSlots,
    onGameModeChange, onLobbyModeChange,
  };
})();
