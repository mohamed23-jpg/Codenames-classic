...
function initSocket() {
  ...
  socket.on("player_left", (data) => {
    ...
    // الصوت يُشغَّل فقط للاعبين الآخرين (ليس للمغادر نفسه)
    const me = App.getPlayer();
    if (data.playerId !== me?.playerId) {
      Audio.play("leave");
    }
  });
  ...
  // طرد من الغرفة
  socket.on("kicked_from_slot", (data) => {
    UI.toast(`تم طردك من المنصب من قِبَل ${data.byNickname || "مدير الغرفة"}`, "error");
    Audio.play("leave");
  });
  socket.on("kicked_from_room", (data) => {
    UI.toast(`تم طردك من الغرفة`, "error", 4000);
    Audio.play("leave");
    setTimeout(() => {
      if (typeof Game !== "undefined") Game.leaveLobby?.();
    }, 1500);
  });
